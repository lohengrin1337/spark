...
Initializes scooters, routes, and charging zones, then starts the main simulation loop.

Advances the scooter positions, updates status and battery, and publishes state to
Redis at each tick ( == UPDATE_INTERVAL).

I hope that we can achieve greater decoupling later, so that the simulator can run independently
in a more straightforward manner, with different inputs, enabling convenient creation of new,
...
"""
Create scooter instances for the simulation with the proper initial state(s).
"""
...
"""
Start the simulation, updating scooter positions, status, battery, and publishing to/through Redis.
"""
def run():
    ...
    print(f"{len(scooters)} scooters active")
