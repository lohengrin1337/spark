...
This the core Simulator is kept clean while still (hopefully) allowing varied, per-scooter
...
def special_malmoe_behavior_one(scooter, trip_count, simulator):
    ...
    if scooter.id == 3 and trip_count >= 2 and simulator.charging_zones:
        ...
        def stand_still(s, dt):
            """This scooter is now permanently parked for this simulation."""
            return {
                "lat": s.lat,
                "lon": s.lon,
                "speed_kmh": 0,
