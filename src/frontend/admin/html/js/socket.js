...
export function initWebSocket() {
  ...
  ws.onopen = () => {
    ...
    fetch('/scooters')
      .then(r => r.json())
      .then(list => {
        list.forEach(sc => pending.set(sc.id, sc));
        applyUpdates();
        document.getElementById('statsContent').innerHTML = "Live - now receiving updates";
      }); */
  };
  ...
  ws.onmessage = message => {
    try {
      ...
      if (msg.id !== undefined) {
        ...
        // Throttle the updates to avoid flooding the map (currently at 1000ms, and currently redundant,
