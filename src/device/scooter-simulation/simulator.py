...
class Simulator:
    """
    Simulator handles scooter movement along predefined routes, manages rentals, and implements
    (injects) a special first scenario where a scooter stops after 2 trips and starts charging inside
    a charging zone.
    ...
    def __init__(self, scooters, routes, charging_zones=None, rbroadcast: ScooterBroadcaster = None,
        ...
        # Current position in the route (index of the next waypoint)
        self.route_state = {scooter.id: {"route_index": 0} for scooter in scooters}
        ...
        # Scenarios can install a custom movement function here — only affects one scooter
    ...
    # -------------------------------------------------------
    # "Back again"-logic: forward on even trips, reversed on odd
    # -------------------------------------------------------
    ...
    # -------------------------------------------------------
    # Main simulation tick — the heartbeat of the simulation of the fleet
    # -------------------------------------------------------
    def tick(self):
    ...
    # -------------------------------------------------------
    # Decide how a scooter should move this tick
    # -------------------------------------------------------
    def _resolve_movement_for_scooter(self, scooter, route):
        ...
        # Normal route-following movement
    ...
    def _handle_rental_tick(self, scooter, prev_lat, prev_lon, update, current_time):
        ...
        if update.get("route_finished", False):
            ...
            # ...otherwise a normal trip ending ensues
            self.trip_counter[scooter_id] = next_trip_count
    ...
    def compute_update(self, scooter, route):
        ...
        * Applies realistic slowdown during sharp turns
        ...
        """
        scooter_id = scooter.id
        route_state = self.route_state[scooter_id]
    ...
    def _is_in_charging_zone(self, scooter):
        ...
        if scooter.status == "active":
            return False
        for zone_lat, zone_lon, radius in self.charging_zones:
            if calculate_distance_in_m((scooter.lat, scooter.lon), (zone_lat, zone_lon)) <= radius:
                return True
        return False